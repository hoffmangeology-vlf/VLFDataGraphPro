...
class VLFDataGraphSessionsApp:
    ...
    def _build_menu(self):
        ...
        analysis_menu.add_command(
            label="Signal Envelope Analysis",
            command=self.open_envelope_analysis
        )
    ...
    def show_user_guide(self):
        guide = (
            ...
            "11. Analysis → Signal Envelope Analysis calculates smoothed, RMS or Hilbert envelopes.\n\n"
            "Notes:\n"
    ...
    def open_envelope_analysis(self):
        session = self.get_current_session()
        if session is None or session.df is None:
            messagebox.showinfo(APP_TITLE, "Open and load a data file before running envelope analysis.")
            return

        numeric_columns = list(session.numeric_columns)
        if not numeric_columns:
            messagebox.showinfo(APP_TITLE, "No numeric channels are available in the current session.")
            return

        win = tk.Toplevel(self.root)
        win.title("Signal Envelope Analysis")
        ...
        selected_now = session.selected_columns()
        default_channel = selected_now[0] if selected_now else numeric_columns[0]

        channel_var = tk.StringVar(value=default_channel)
        method_var = tk.StringVar(value="Smoothed absolute envelope")
        window_var = tk.StringVar(value="300")
        remove_mean_var = tk.BooleanVar(value=True)
        normalize_var = tk.BooleanVar(value=False)
        status_var = tk.StringVar(value="Select parameters and click Calculate.")

        ttk.Label(controls, text="Channel:").pack(side=tk.LEFT)
        ttk.Combobox(controls, textvariable=channel_var, values=numeric_columns,
                     state="readonly", width=18).pack(side=tk.LEFT, padx=(4, 12))
        ttk.Label(controls, text="Method:").pack(side=tk.LEFT)
        ttk.Combobox(controls, textvariable=method_var,
                     values=["Smoothed absolute envelope", "RMS envelope", "Hilbert envelope"],
                     state="readonly", width=27).pack(side=tk.LEFT, padx=(4, 12))
        ttk.Label(controls, text="Window [samples]:").pack(side=tk.LEFT)
        ttk.Entry(controls, textvariable=window_var, width=8).pack(side=tk.LEFT, padx=(4, 12))
        ttk.Checkbutton(controls, text="Remove mean", variable=remove_mean_var).pack(side=tk.LEFT)
        ttk.Checkbutton(controls, text="Normalize", variable=normalize_var).pack(side=tk.LEFT, padx=(8, 0))

        graph_frame = ttk.Frame(win)
        graph_frame.pack(fill=tk.BOTH, expand=True, padx=8, pady=(0, 6))
        figure = Figure(figsize=(10, 7), dpi=100)
        ax_signal, ax_envelope = figure.subplots(2, 1, sharex=True)
        canvas = FigureCanvasTkAgg(figure, master=graph_frame)
        canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True)
        nav = NavigationToolbar2Tk(canvas, graph_frame, pack_toolbar=False)
        nav.update(); nav.pack(fill=tk.X)

        bottom = ttk.Frame(win, padding=(8, 0, 8, 8))
        bottom.pack(fill=tk.X)
        ttk.Label(bottom, textvariable=status_var).pack(side=tk.LEFT)

        result = {"x": None, "signal": None, "envelope": None, "channel": None, "method": None, "is_datetime": False}
        ...
        def calculate_envelope():
            try:
                ...
                y = pd.to_numeric(session.df[channel], errors="coerce").to_numpy(dtype=float)
                x, x_label, x_is_datetime = session.get_x_data()
                x = np.asarray(x)
                valid = pd.notna(x) & np.isfinite(y)
                if not valid.any():
                    raise ValueError("The selected channel contains no valid data.")

                x_valid, y_valid = x[valid], y[valid]
                processed = y_valid - np.nanmean(y_valid) if remove_mean_var.get() else y_valid.copy()

                if method == "Smoothed absolute envelope":
                    envelope = pd.Series(np.abs(processed)).rolling(window, center=True, min_periods=1).mean().to_numpy()
                elif method == "RMS envelope":
                    envelope = np.sqrt(pd.Series(processed ** 2).rolling(window, center=True, min_periods=1).mean().to_numpy())
                elif method == "Hilbert envelope":
                    if hilbert is None:
                        raise RuntimeError("Hilbert envelope requires SciPy.\n\nInstall it with:\npython -m pip install scipy")
                    filled = pd.Series(processed).interpolate(limit_direction="both").to_numpy()
                    raw = np.abs(hilbert(filled))
                    envelope = pd.Series(raw).rolling(window, center=True, min_periods=1).mean().to_numpy() if window > 1 else raw
                else:
                    raise ValueError("Unknown envelope method.")

                if normalize_var.get():
                    maximum = np.nanmax(np.abs(envelope))
                    if maximum > 0:
                        envelope = envelope / maximum

                xs, ys = reduce_minmax(x_valid, y_valid, max_points=80000)
                xe, ye = reduce_minmax(x_valid, envelope, max_points=80000)
                ax_signal.clear(); ax_envelope.clear()
                ax_signal.plot(xs, ys, linewidth=0.7, label=channel)
                ax_signal.set_ylabel("Signal"); ax_signal.set_title(f"{channel} – original signal")
                ax_signal.grid(True, alpha=0.3); ax_signal.legend(loc="best", fontsize=8)
                ax_envelope.plot(xe, ye, linewidth=1.0, label=method)
                ax_envelope.set_ylabel("Envelope"); ax_envelope.set_xlabel(x_label); ax_envelope.set_title(method)
                ax_envelope.grid(True, alpha=0.3); ax_envelope.legend(loc="best", fontsize=8)
                if x_is_datetime:
                    ax_envelope.xaxis.set_major_locator(mdates.HourLocator(byhour=[0, 12]))
                    ax_envelope.xaxis.set_major_formatter(mdates.DateFormatter("%d.%m.%Y\n%H:%M"))
                figure.tight_layout(); canvas.draw_idle()

                result.update(x=x_valid, signal=y_valid, envelope=envelope, channel=channel,
                              method=method, is_datetime=x_is_datetime)
                status_var.set(f"Done. Samples: {len(y_valid):,} | Envelope min: {np.nanmin(envelope):.4g} | mean: {np.nanmean(envelope):.4g} | max: {np.nanmax(envelope):.4g}")
            except Exception as exc:
                messagebox.showerror("Envelope Analysis", str(exc), parent=win)

        def export_envelope():
            if result["envelope"] is None:
                messagebox.showinfo("Envelope Analysis", "Calculate an envelope first.", parent=win)
                return
            default_name = f"{Path(session.file_path).stem}_{result['channel']}_envelope.csv"
            path = filedialog.asksaveasfilename(parent=win, title="Export Envelope",
                                                initialfile=default_name, defaultextension=".csv",
                                                filetypes=[("CSV", "*.csv")])
            if not path:
                return
            x_name = "datetime" if result["is_datetime"] else "x"
            x_output = pd.to_datetime(result["x"]) if result["is_datetime"] else result["x"]
            pd.DataFrame({x_name: x_output, result["channel"]: result["signal"],
                          "envelope": result["envelope"], "method": result["method"]}).to_csv(
                              path, index=False, sep=";", decimal=",")
            status_var.set(f"Envelope exported: {path}")

        def save_envelope_graph():
            path = filedialog.asksaveasfilename(parent=win, title="Save Envelope Plot",
                                                defaultextension=".png",
                                                filetypes=[("PNG", "*.png"), ("PDF", "*.pdf"), ("SVG", "*.svg")])
            if path:
                figure.savefig(path, dpi=300, bbox_inches="tight")
                status_var.set(f"Envelope plot saved: {path}")

        ttk.Button(controls, text="Calculate", command=calculate_envelope).pack(side=tk.LEFT, padx=(14, 0))
        ttk.Button(controls, text="Export CSV", command=export_envelope).pack(side=tk.LEFT, padx=(5, 0))
        ttk.Button(controls, text="Save Plot", command=save_envelope_graph).pack(side=tk.LEFT, padx=(5, 0))
        calculate_envelope()
